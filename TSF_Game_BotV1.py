...
def get_bot_guess(chars, bot_guess, num_digits, bot_clues, corrects):
    new_bot_guess = []
    ...
    for i in range(num_digits):
        valid_corrects = [item for item in corrects if item not in new_bot_guess]
        valid_chars = [item for item in chars if item not in new_bot_guess]
...
def get_bot_clues(chars, bot_guess, bot_secret_num, corrects):
    bot_clues = []
    for i in range(len(bot_guess)):
        if str(bot_guess[i]) == str(bot_secret_num[i]):
            bot_clues.append('T')
            if bot_guess[i] in chars:
                chars.remove(bot_guess[i])
...
def main():
    ...
    while True:
        ...
        bot_guess = random.sample(chars, num_digits)
        print(f'Bot guess #{num_guesses}: ', bot_guess)
        bot_clues = get_bot_clues(chars, bot_guess, bot_secret_num, corrects)
        ...
        while True:
            ...
            bot_guess = get_bot_guess(chars, bot_guess, num_digits, bot_clues, corrects)
            print(f'Bot guess #{num_guesses}: ', bot_guess)
            bot_clues = get_bot_clues(chars, bot_guess, bot_secret_num, corrects)
            print(f'Bot clues #{num_guesses}: ', bot_clues)
            print()
